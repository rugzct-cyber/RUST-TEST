...
use super::types::{extended_symbol_to_canonical, get_extended_symbols, ExtendedOrderbookMsg};
...
impl ExtendedAdapter {
    ...
    async fn subscribe_to_orderbooks(&self) -> ExchangeResult<()> {
        ...
        let symbols: Vec<&str> = get_extended_symbols().iter().map(|(s, _)| *s).collect();
        let msg = serde_json::json!({"type": "subscribe", "channel": "orderbooks", "depth": 1, "symbols": symbols});
        let mut sender = ws_sender.lock().await;
        sender.send(Message::Text(msg.to_string())).await.map_err(|e| ExchangeError::WebSocket(Box::new(e)))?;
        tracing::info!(exchange = "extended", count = symbols.len(), "Subscribed to orderbooks depth=1");
        Ok(())
    }
        ...
        while let Some(msg_result) = ws_receiver.next().await {
            ...
            match msg_result {
                Ok(Message::Text(text)) => {
                    msg_count += 1;
                    if msg_count <= 3 {
                        tracing::info!(exchange = "extended", msg_count, raw = %text.chars().take(300).collect::<String>(), "RAW WS message");
                    }
                    if let Ok(msg) = serde_json::from_str::<ExtendedOrderbookMsg>(&text) {
                        let msg_type = match &msg.msg_type { Some(t) => t.clone(), None => continue };
                        if msg_type != "SNAPSHOT" && msg_type != "DELTA" { continue; }
                        let data = match msg.data { Some(d) => d, None => continue };
                        let ext_symbol = match &data.m { Some(s) => s.clone(), None => continue };
                        if let Some(symbol) = extended_symbol_to_canonical(&ext_symbol) {
                            let bid = data.b.as_ref().and_then(|levels| levels.first()).and_then(|l| l.p.parse::<f64>().ok());
                            let ask = data.a.as_ref().and_then(|levels| levels.first()).and_then(|l| l.p.parse::<f64>().ok());
                ...
                Ok(_) => {}
                Err(e) => { tracing::error!("Extended WS error: {}", e); break; }
            }
        }
            ...
            loop {
                interval.tick().await;
                {
                    ...
                    if let Err(_) = sender.send(Message::Ping(vec![])).await {
                        reader_alive.store(false, Ordering::Relaxed); break;
                    }
                }
...
impl ExchangeAdapter for ExtendedAdapter {
    ...
    async fn reconnect(&mut self) -> ExchangeResult<()> {
        ...
        Err(ExchangeError::ConnectionFailed("Extended reconnection failed".into()))
    }
    fn exchange_name(&self) -> &'static str { "extended" }
