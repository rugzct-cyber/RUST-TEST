...
///
/// Factored out to avoid drift between `sign_auth_message` and `sign_order_message`.
fn compute_paradex_domain_hash(
    chain_felt: starknet_core::types::Felt,
) -> starknet_core::types::Felt {
    use starknet_core::types::Felt;
    let domain_type_hash =
        starknet_keccak("StarkNetDomain(name:felt,chainId:felt,version:felt)".as_bytes());
    let domain_name =
        cairo_short_string_to_felt("Paradex").expect("'Paradex' is always a valid short string");
...
pub fn sign_auth_message(
    ...
    // === Build Domain hash (factored into helper) ===
...
#[cfg(test)]
mod tests {
    use super::super::config::{TEST_ACCOUNT_ADDRESS, TEST_PRIVATE_KEY};
    use super::*;
