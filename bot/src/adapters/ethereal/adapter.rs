//! Ethereal Adapter — BookDepth channel
...
use super::config::EtherealConfig;
use super::types::{get_ethereal_markets, product_id_to_symbol, EtherealWsMessage};
...
impl EtherealAdapter {
    pub fn new(config: EtherealConfig) -> Self {
        Self {
    ...
    async fn connect_websocket(&mut self) -> ExchangeResult<()> {
        ...
        tracing::info!("Connecting to Ethereal WebSocket: {}", url);
    ...
    async fn subscribe_to_book_depth(&self) -> ExchangeResult<()> {
        ...
        let markets = get_ethereal_markets();
        for (product_id, _) in &markets {
            let msg = serde_json::json!({"type": "subscribe", "channel": "BookDepth", "product_id": product_id});
            let mut sender = ws_sender.lock().await;
            sender.send(Message::Text(msg.to_string())).await.map_err(|e| ExchangeError::WebSocket(Box::new(e)))?;
            tokio::time::sleep(Duration::from_millis(50)).await;
        }
        tracing::info!(exchange = "ethereal", count = markets.len(), "Subscribed to BookDepth");
        Ok(())
    }
        ...
        while let Some(msg_result) = ws_receiver.next().await {
            ...
            match msg_result {
                Ok(Message::Text(text)) => {
                    if let Ok(msg) = serde_json::from_str::<EtherealWsMessage>(&text) {
                        if let Some(data) = msg.data {
                            let product_id = match &data.product_id { Some(p) => p.clone(), None => continue };
                            if let Some(symbol) = product_id_to_symbol(&product_id) {
                                let bid: f64 = match data.bid_price.as_ref().and_then(|s| s.parse().ok()) { Some(v) => v, None => continue };
                                let ask: f64 = match data.ask_price.as_ref().and_then(|s| s.parse().ok()) { Some(v) => v, None => continue };
                                if bid > 0.0 && ask > 0.0 {
                ...
                Ok(_) => {}
                Err(e) => { tracing::error!("Ethereal WS error: {}", e); break; }
            }
        }
...
impl ExchangeAdapter for EtherealAdapter {
    ...
    async fn reconnect(&mut self) -> ExchangeResult<()> {
        ...
        Err(ExchangeError::ConnectionFailed("Ethereal reconnection failed".into()))
    }
    fn exchange_name(&self) -> &'static str { "ethereal" }
...
mod tests {
    ...
    fn test_adapter_basics() {
        let a = EtherealAdapter::new(EtherealConfig::default());
        assert!(!a.connected);
        assert_eq!(a.exchange_name(), "ethereal");
    }
}
