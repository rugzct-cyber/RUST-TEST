//! Reya Adapter Implementation
//!
//! WebSocket adapter for Reya DEX using /v2/prices channel.
//! poolPrice = bid, oraclePrice = ask.
...
use super::config::ReyaConfig;
use super::types::{reya_symbol_to_canonical, ReyaWsMessage};
...
impl ReyaAdapter {
    pub fn new(config: ReyaConfig) -> Self {
        Self {
    ...
    async fn connect_websocket(&mut self) -> ExchangeResult<()> {
        ...
        tracing::info!("Connecting to Reya WebSocket: {}", url);
    ...
    async fn message_reader_loop(
        ...
        tracing::info!("Reya message_reader_loop started");
        ...
        while let Some(msg_result) = ws_receiver.next().await {
            ...
            match msg_result {
                Ok(Message::Text(text)) => {
                    msg_count += 1;
                    if msg_count <= 3 {
                        tracing::info!(exchange = "reya", msg_count, raw = %text.chars().take(300).collect::<String>(), "RAW WS message");
                    }
                    if let Ok(msg) = serde_json::from_str::<ReyaWsMessage>(&text) {
                        let items = match msg {
                            ReyaWsMessage::ChannelData { data } => data,
                            ReyaWsMessage::Subscribed { contents } => contents,
                            ReyaWsMessage::Pong => { tracing::trace!("Reya PONG"); continue; }
                        };

                        for item in items {
                            if let Some(ref sym_str) = item.symbol {
                                if let Some(symbol) = reya_symbol_to_canonical(sym_str) {
                                    let bid: f64 = match item.pool_price.parse() { Ok(v) => v, Err(_) => continue };
                                    let ask: f64 = match item.oracle_price.parse() { Ok(v) => v, Err(_) => continue };
                                    if bid > 0.0 && ask > 0.0 {
                ...
                Ok(Message::Close(_)) => { tracing::info!("Reya WS closed"); break; }
                Ok(_) => {}
                Err(e) => { tracing::error!("Reya WS error: {}", e); break; }
            }
        }
        reader_alive.store(false, Ordering::Relaxed);
        tracing::warn!("Reya message reader loop ended");
    }
            ...
            loop {
                ...
                let ping = serde_json::json!({"type": "ping"});
                {
                    ...
                    if let Err(e) = sender.send(Message::Text(ping.to_string())).await {
                        tracing::warn!("Reya heartbeat: ping failed - {}", e);
                        reader_alive.store(false, Ordering::Relaxed);
                        break;
                    }
                }
                ...
                if age > 60_000 {
                    tracing::warn!("Reya heartbeat: stale ({}ms)", age);
                    reader_alive.store(false, Ordering::Relaxed);
                    break;
                }
            }
...
#[async_trait]
impl ExchangeAdapter for ReyaAdapter {
    async fn connect(&mut self) -> ExchangeResult<()> {
        self.connect_websocket().await?;
        self.split_and_spawn_reader()?;
        self.spawn_heartbeat_task();
        self.subscribe_to_prices().await?;
        self.connected = true;
        tracing::info!(exchange = "reya", "Reya WebSocket connected");
        Ok(())
    }
    ...
    async fn reconnect(&mut self) -> ExchangeResult<()> {
        tracing::info!("Reya: Reconnecting...");
        ...
        { let mut state = self.connection_health.state.write().await; *state = ConnectionState::Disconnected; }
        Err(ExchangeError::ConnectionFailed("Reya reconnection failed".into()))
    }

    fn exchange_name(&self) -> &'static str { "reya" }
...
mod tests {
    ...
    fn test_adapter_not_connected_initially() {
        let adapter = ReyaAdapter::new(ReyaConfig::default());
        assert!(!adapter.connected);
        assert_eq!(adapter.exchange_name(), "reya");
    }
}
