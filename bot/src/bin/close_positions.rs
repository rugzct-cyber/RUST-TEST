...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Connect both adapters in parallel (shared init)
    let (vest_adapter, mut paradex_adapter) = hft_bot::bin_utils::connect_adapters().await?;
    ...
    match &vest_pos {
        Some(pos) => {
            info!(exchange = "vest", side = %pos.side.to_uppercase(), qty = pos.quantity, entry_price = pos.entry_price, "Position found")
        }
        None => info!(exchange = "vest", "No position"),
    }
    match &paradex_pos {
        Some(pos) => {
            info!(exchange = "paradex", side = %pos.side.to_uppercase(), qty = pos.quantity, entry_price = pos.entry_price, "Position found")
        }
    ...
    let vest_close = async {
        if let Some(pos) = vest_pos {
            ...
            let close_side = if pos.side.to_lowercase() == "long" {
                OrderSide::Sell
            } else {
                OrderSide::Buy
            };

            // Use mark_price (current market) for slippage baseline, fallback to entry_price
            let base_price = pos.mark_price.unwrap_or(pos.entry_price);
            let price_source = if pos.mark_price.is_some() {
                "mark_price"
            } else {
            ...
            Err(last_err.expect("retry loop must set last_err on failure"))
        } else {
    ...
    let paradex_close = async {
        if let Some(pos) = paradex_pos {
            ...
            let close_side = if pos.side.to_lowercase() == "short" {
                OrderSide::Buy
            } else {
                OrderSide::Sell
            };
    ...
    match vest_adapter.get_position(&vest_pair).await {
        Ok(Some(pos)) => {
            warn!(exchange = "vest", side = %pos.side, qty = pos.quantity, "Position still open")
        }
    ...
    match paradex_adapter.get_position(&paradex_pair).await {
        Ok(Some(pos)) => {
            warn!(exchange = "paradex", side = %pos.side, qty = pos.quantity, "Position still open")
        }
        Ok(None) => info!(exchange = "paradex", "Position closed"),
        Err(e) => error!(exchange = "paradex", error = %e, "Error checking position"),
    }

    // Disconnect adapters on shutdown
    let _ = paradex_adapter.disconnect().await;
    info!("=== CLOSE COMPLETE ===");
    Ok(())
}
